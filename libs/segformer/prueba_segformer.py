...
class SegformerDANA(nn.Module):
    def __init__(self, token):
        ...
        self.model = SegformerForSemanticSegmentation.from_pretrained("nvidia/segformer-b0-finetuned-ade-512-512", token = token)
...
CHECKPOINT_PATH = os.path.join(BASE_DIR, "/home/salvem/hugoa/output/segformer.pth") 
...
class SegformerInference:
    def __init__(self, device: str):        
        ...
        self.predictor.load_state_dict(torch.load(CHECKPOINT_PATH, weights_only=True))
        self.predictor = self.predictor.to(device)
        self.device = device
        self.processor = SegformerImageProcessor.from_pretrained("nvidia/segformer-b0-finetuned-ade-512-512", token = secret["hf"])
    ...
    def get_mask(self, image: np.ndarray):
        ...
        image_tensor = self.processor(images=image.resize((512, 512)), return_tensors="pt").pixel_values.reshape((-1, 3, 512, 512)).to(self.device)
...
if __name__ == "__main__":
  detector = SegformerInference("cuda:1")
  array = np.array(Image.open("/home/salvem/hugoa/segmentation/images/0013ER-S1- (15).jpg"))
  print(array.shape)
  #array = np.expand_dims(array, axis = 0)
  print(array.shape)
  mask = Image.fromarray(detector.get_mask(array))
  mask.save("prueba.png")
