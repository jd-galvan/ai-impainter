...
class UNetInference:
    ...
    def get_mask(self, image: np.ndarray):
        ...
        image_tensor = self.transformations(image).reshape((1, 3, 1024, 1024)).to(self.device)
    
        # Predecir la máscara
        mask = self.predictor(image_tensor)
        mask = torch.sigmoid(mask)
       
        # Binarizar la máscara
        mask_bin = (mask >= 0.2).float() #Threshold 0.2
